...
void RecycleDictionary(LZWExp *exp) {
   int ndx = 0;
...
int LZWExpDecode(LZWExp *exp, UInt bits) { 
   ...
   while (unpackStatus && !exp->EODSeen && ret <= exp->lastCode) {
      ...
      if (unpackStatus && exp->lastCode == exp->recycleCode){
         RecycleDictionary(exp);
      }
